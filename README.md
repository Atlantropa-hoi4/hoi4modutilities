...
| `hoi4ModUtilities.formatter.ignoreFiles` | `string[]` | Workspace-folder-relative glob patterns excluded from document, range, on-type, and workspace formatting. Use forward slashes, such as `common/generated/**` or `**/legacy.txt`. The default list skips vanilla-derived folders: `interface`, `gfx`, `common/names`, `common/occupation_laws`, `common/special_projects`, `common/technologies`, `common/units` (except `names_divisions` and `names_ships`), and `history/states`. Setting this value replaces the default list. |
