...
# FocusTree Pending Placeholder Rendering Fix 2026-04-12

## Plan
- [x] 새로 생성된 focus가 어떤 placeholder 경로로 즉시 렌더되는지 다시 확인한다
- [x] webview CSP를 깨지 않는 방식으로 pending placeholder 마크업을 정상 카드 형태로 보이게 수정한다
- [x] placeholder 렌더 회귀 테스트를 추가하거나 갱신한다
- [x] 관련 unit test를 실행해 수정이 실제로 동작하는지 검증한다

## Review
- 원인은 `webviewsrc/focustree.ts`의 optimistic create 경로가 `src/previewdef/focustree/localpreview.ts`의 pending placeholder HTML을 그대로 쓰는 동안, 그 placeholder가 전부 inline `style=""` 속성에 의존하던 점이었다. 이 preview는 nonce-based CSP를 사용하므로 inline style이 차단되어 새 focus가 카드가 아니라 평문 ID처럼 보였다.
- pending placeholder는 이제 inline style 대신 SVG 속성만으로 배경, 아이콘 슬롯, 텍스트를 그리는 CSP-safe 템플릿을 사용한다. 그래서 host의 정상 `renderedFocus` 패치가 도착하기 전에도 새 focus가 실제 카드와 비슷한 형태로 즉시 보인다.
- `test/unit/focustree-localpreview.test.ts`는 새 placeholder가 SVG 기반 마크업을 쓰고 inline style을 다시 도입하지 않도록 회귀 기대값을 갱신했다.

## Verification
- `npm run compile-tests` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-localpreview.test.js` passed with 3 tests.
- `npm run compile-ts` passed.

# FocusTree Icon Refresh Fix 2026-04-12

## Plan
- [x] focus icon 이미지가 host/webview 사이에서 어떤 스타일 의존성으로 갱신되는지 다시 확인한다
- [x] `.gfx` 등 아이콘 자산 변경 시 stale dynamic CSS를 재사용하지 않도록 patch 판단을 보정한다
- [x] 아이콘 자산 변경이 full render로 승격되는 회귀 테스트를 추가한다
- [x] 관련 테스트와 타입 검증을 실행하고 결과를 기록한다

## Review
- `src/previewdef/focustree/renderpayloadpatch.ts`의 style dependency signature는 이제 `gfxFiles`도 함께 본다. 덕분에 focus 구조나 icon key 이름이 그대로여도, 실제 sprite를 결정하는 `.gfx` 의존성이 바뀌면 partial patch를 재사용하지 않고 full snapshot으로 승격된다.
- 이 보정은 "회색 placeholder CSS를 가진 예전 동적 스타일 블록"을 새 sprite 자산이 생긴 뒤에도 계속 유지하던 경우를 막는다. 즉 아이콘 자산 해석 결과가 달라지는 순간 webview의 `dynamicStyleCss` 전체가 다시 교체된다.
- `test/unit/focustree-renderpayloadpatch.test.ts`에 icon asset dependency가 `interface/old.gfx`에서 `interface/new.gfx`로 바뀌면 full render가 필요하다는 회귀 테스트를 추가했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-renderpayloadpatch.test.js` passed as part of the focused mocha run below.

# FocusTree Localization Label Display 2026-04-12

## Plan
- [x] 현재 focus 카드 텍스트 렌더 경로와 사용 가능한 localisation key 소스를 다시 확인한다
- [x] focus 코드 아래에 localisation 라인을 분리 표시하도록 템플릿을 보정한다
- [x] localisation 표시 회귀 테스트를 추가한다
- [x] 관련 타입/단위 테스트를 실행하고 결과를 기록한다

## Review
- `src/previewdef/focustree/focusrender.ts`에 `resolveFocusLocalizationText()`를 추가해, focus 카드가 표시할 두 번째 줄 텍스트를 한 곳에서 결정하도록 정리했다. 우선순위는 `focus.id`의 resolved localisation, 그다음 explicit `text = ...` key의 resolved localisation, 마지막으로 인덱스가 아직 준비되지 않았을 때의 raw `focus.text` key다.
- 같은 파일의 `renderFocusHtmlTemplate()`는 이제 코드 줄과 localisation 줄을 분리된 `<span>`으로 렌더한다. 그래서 preview 카드에서 focus 코드 아래에 localisation 문구가 명시적으로 한 줄 더 보이고, 스타일도 별도로 조정할 수 있게 됐다.
- `test/unit/focustree-focusrender.test.ts`를 추가해 direct-id localisation, explicit text-key fallback, 실제 템플릿에 localisation line이 들어가는 경우를 회귀 테스트로 고정했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-focusrender.test.js out\\test\\unit\\focustree-renderpayloadpatch.test.js` passed with 7 tests.

# FocusTree Early Icon Hydration 2026-04-12

## Plan
- [x] deferred 첫 스냅샷에서 focus icon이 왜 회색 placeholder로 먼저 보이는지 경로를 다시 확인한다
- [x] shell-first 초기화는 유지하면서 focus icon만 첫 실제 스냅샷에서 바로 준비되도록 수정한다
- [x] deferred payload에서도 실제 icon CSS가 들어가는 회귀 테스트를 추가한다
- [x] 관련 타입/단위 테스트를 실행하고 결과를 기록한다

## Review
- 원인은 `initializePanel()` 이후 첫 실제 snapshot이 `assetLoadMode: deferred`일 때, `loader.ts`와 `contentbuilder.ts`가 focus icon 자산도 함께 defer해서 회색 placeholder CSS를 먼저 보내고, 나중 full hydration에서야 실제 sprite CSS를 붙이던 구조였다.
- `src/previewdef/focustree/loader.ts`는 이제 deferred 모드에서도 focus icon name 기준 `.gfx` 의존성은 계속 수집한다. 그래서 첫 snapshot부터 mod/vanilla icon을 해석하는 데 필요한 gfx 파일 목록이 준비된다.
- `src/previewdef/focustree/contentbuilder.ts`는 이제 deferred 여부와 관계없이 focus icon 스타일은 바로 생성한다. 즉 shell-first는 유지하지만, focus card의 핵심인 icon은 첫 `focusTreeContentUpdated`부터 실제 sprite CSS를 사용하고, inlay window 같은 부가 자산만 후속 hydration에 남긴다.
- `test/unit/focustree-contentbuilder.test.ts`를 추가해 deferred base state에서도 `dynamicStyleCss`에 실제 icon `background-image`가 포함되는 계약을 고정했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-contentbuilder.test.js out\\test\\unit\\focustree-focusrender.test.js out\\test\\unit\\focustree-renderpayloadpatch.test.js` passed with 8 tests.

# FocusTree Localization Key Priority Fix 2026-04-12

## Plan
- [x] focus preview가 title localisation key를 어떤 우선순위로 고르는지 다시 확인한다
- [x] explicit `text = ...` key가 있으면 `id` localisation보다 우선 사용하도록 보정한다
- [x] 해당 우선순위 회귀 테스트를 갱신한다
- [x] 관련 테스트를 실행하고 결과를 기록한다

## Review
- 원인은 `src/previewdef/focustree/focusrender.ts`의 localisation 선택 순서가 HOI4 title 규칙과 반대였던 점이다. 기존 로직은 `focus.id`의 번역을 먼저 보고, 그 뒤에야 explicit `text = ...` key를 봤기 때문에 `text`가 title override인 focus에서 서로 맞지 않는 문구가 아래 줄에 붙을 수 있었다.
- `resolveFocusLocalizationText()`는 이제 explicit `text` key를 먼저 해석하고, 그 키가 실제로 title override라면 `id` localisation보다 우선 사용한다. explicit key가 아직 번역되지 않았으면 raw `focus.text` key를 fallback으로 보여 주고, explicit override가 없을 때만 `id` localisation을 쓴다.
- `test/unit/focustree-focusrender.test.ts`는 explicit text-key 우선, id fallback, raw text-key fallback, 실제 템플릿 렌더를 각각 검증하도록 갱신했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-focusrender.test.js` passed with 4 tests.

# FocusTree Inlay Visibility And First Paint Regression Fix 2026-04-12

## Plan
- [x] inlay window 선택기가 왜 남아 있는지 현재 toolbar 노출 조건과 초기 shell 마크업을 다시 확인한다
- [x] 실제로 보이는 inlay window가 없으면 선택기가 뜨지 않도록 기본 숨김과 runtime 표시 조건을 함께 보정한다
- [x] deferred 첫 스냅샷이 focus icon 자산 해석을 기다리지 않도록 빠른 placeholder 경로를 복원한다
- [x] 관련 unit test를 갱신하고 compile/test로 회귀 여부를 검증한다

## Review
- `webviewsrc/focustree.ts`는 이제 inlay 선택기 표시 여부를 `focusTree.inlayWindows.length`가 아니라 실제 조건 필터를 통과한 `visibleInlayWindows.length`로 결정한다. 그래서 현재 트리에서 표시 가능한 inlay가 없으면 selector row가 남지 않는다.
- `src/previewdef/focustree/contentbuilder.ts`는 shell 마크업에서 `#inlay-window-container`를 기본 `display:none`으로 시작하게 바꿨다. 덕분에 webview bootstrap 중 잠깐이라도 오래된 selector가 먼저 보이는 깜빡임이 줄어든다.
- 같은 파일의 deferred payload 경로는 다시 `prepareDeferredFocusIconStyles()`를 사용하도록 복원했다. 즉 첫 실제 snapshot은 회색 placeholder icon CSS만 빠르게 만들고, 무거운 sprite 해석은 full asset 경로에서 처리한다.
- `src/previewdef/focustree/loader.ts`도 deferred 모드에서는 focus icon용 `.gfx` 탐색을 건너뛰도록 되돌렸다. 이 변경으로 첫 focus 노드 표시가 icon asset scan에 막히지 않게 된다.
- `test/unit/focustree-contentbuilder.test.ts`는 deferred payload가 실제 icon URI 대신 lightweight placeholder CSS를 유지하는지, 그리고 shell HTML이 inlay selector를 기본 숨김으로 내보내는지를 회귀 테스트로 고정했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-contentbuilder.test.js out\\test\\unit\\focustree-focusrender.test.js out\\test\\unit\\focustree-renderpayloadpatch.test.js` passed with 10 tests.

# FocusTree Parallel Icon Hydration Fix 2026-04-12

## Plan
- [x] deferred 첫 스냅샷과 full hydration이 현재 어떤 순서로 실행되는지 previewsession 경로를 다시 확인한다
- [x] 첫 focus paint는 유지하면서 full hydration base state를 병렬로 미리 준비하도록 수정한다
- [x] hydration 단계가 이미 시작한 full build 결과를 재사용하는 회귀 테스트를 추가한다
- [x] 관련 compile/test를 다시 실행해 startup과 patch 경로를 함께 검증한다

## Review
- 원인은 `src/previewdef/focustree/previewsession.ts`에서 deferred 첫 snapshot을 webview에 보낸 뒤에야 full hydration refresh를 새로 시작하던 점이었다. 그래서 빠른 첫 paint를 되살리면 되살릴수록 icon sprite가 붙는 시점은 항상 그 다음 full build만큼 더 늦어졌다.
- 같은 파일은 이제 deferred refresh를 시작할 때 full `buildBaseState(..., 'full')`도 병렬로 바로 시작하고, 그 promise를 runtime state에 보관한다. 이후 첫 deferred update를 보낸 뒤에는 새 full refresh를 다시 만들지 않고, 이미 돌고 있던 full base state 결과를 받아 바로 hydration snapshot으로 이어 간다.
- 이 변경으로 focus 노드 자체는 여전히 빠른 placeholder 경로로 먼저 보이지만, icon hydration은 "deferred render 완료 후 다시 full build 시작"의 직렬 지연을 먹지 않게 된다. 즉 첫 paint 지연과 icon 지연을 같은 스위치로 번갈아 바꾸던 구조를 분리했다.
- `test/unit/focustree-previewsession.test.ts`에는 initialize 시점에 `buildBaseState('full')`과 `buildBaseState('deferred')`가 함께 시작되고, hydration 단계에서 추가 build 없이 기존 full promise를 재사용해 두 번째 update를 보내는 회귀 테스트를 추가했다. 단독 실행이 가능하도록 `vscode` mock도 함께 보강했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-previewsession.test.js out\\test\\unit\\focustree-contentbuilder.test.js out\\test\\unit\\focustree-focusrender.test.js out\\test\\unit\\focustree-renderpayloadpatch.test.js` passed with 19 tests.

# FocusTree Placeholder Editability And Refresh Cache Fix 2026-04-12

## Plan
- [x] 새 focus 이동 불가와 IDE 변경 미반영 경로를 optimistic placeholder, host refresh, webview layout cache 기준으로 다시 확인한다
- [x] 새 placeholder focus가 authoritative refresh 전에도 드래그 가능하도록 editable metadata를 보강한다
- [x] host tree replacement/patch 적용 시 stale layout cache가 남지 않도록 webview 측 invalidation을 추가한다
- [x] 관련 edit/local preview/session 회귀 테스트와 compile/test를 실행한다

## Review
- `src/previewdef/focustree/localpreview.ts`의 `createPlaceholderFocus()`는 이제 pending focus에도 최소 `layout` 메타데이터를 채운다. 그리고 placeholder SVG 템플릿도 `data-focus-editable="true"`로 렌더되도록 바꿨다. 덕분에 새로 만든 focus는 authoritative host refresh가 도착하기 전에도 edit mode에서 바로 드래그 대상이 된다.
- `webviewsrc/focustree.ts`의 `setFocusTrees`와 `applyFocusTreePatches`는 이제 기존/신규 tree 객체에 대해 `invalidateCachedFocusTreeLayoutPlan()`을 먼저 호출한다. 그래서 IDE 수정이나 host patch가 들어왔을 때 이전 tree 객체 기준으로 계산된 layout plan이 남아 화면이 안 바뀌는 상황을 막는다.
- `test/unit/focustree-localpreview.test.ts`는 pending placeholder가 editable metadata를 가지는지 고정했고, `test/unit/focustree-edithandler.test.ts`는 실제 위치 이동 command가 workspace edit 적용 후 optimistic ack와 reconcile을 보내는지 검증하도록 확장했다.
- 추가로 `test/unit/focustree-previewsession.test.ts` 회귀를 유지해, 초기 hydration이 진행 중이어도 이후 IDE 문서 refresh가 최신 버전으로 preview에 반영되는 경로를 계속 고정했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-localpreview.test.js out\\test\\unit\\focustree-edithandler.test.js out\\test\\unit\\focustree-previewsession.test.js out\\test\\unit\\focustree-contentbuilder.test.js out\\test\\unit\\focustree-focusrender.test.js out\\test\\unit\\focustree-renderpayloadpatch.test.js` passed with 28 tests.

# FocusTree Completion Reward Template Spacing Fix 2026-04-12

## Plan
- [x] 새 focus 생성 템플릿에서 `completion_reward` 블록을 만드는 위치를 확인한다
- [x] 블록 내부에 빈 줄이 들어가도록 nested/top-level 템플릿을 함께 수정한다
- [x] 관련 생성 테스트를 실행하고 결과를 기록한다

## Review
- `src/previewdef/focustree/positioneditservicetext.ts`의 nested/top-level focus 템플릿 생성부는 모두 `completion_reward = {}`를 바로 닫는 형태였다.
- 두 템플릿 모두 여는 중괄호 다음에 `lineEnding` 하나를 더 넣어, 새로 생성되는 focus가 `completion_reward = {` 다음 빈 줄을 가진 상태로 삽입되도록 맞췄다.
- `test/unit/focustree-positionedit.test.ts`도 `completion_reward` 블록 사이에 실제 빈 줄이 있어야만 통과하도록 정규식을 조여, 다시 한 줄짜리 블록으로 돌아가는 회귀를 막았다.

## Verification
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-positionedit.test.js` passed with 28 tests.

# FocusTree Active File Sync Fix 2026-04-12

## Plan
- [x] shell-first preview 이후 webview가 current editable file 정보를 계속 들고 있는지 확인한다
- [x] snapshot update가 `focusPositionActiveFile`을 함께 전달하고 webview 상태에 반영되도록 수정한다
- [x] continuous position/editability 회귀 테스트를 보강한다
- [x] compile/test 결과를 기록한다

## Review
- 원인은 shell-first 초기화 이후 실제 `focusTreeContentUpdated` 메시지가 `focusPositionActiveFile`을 다시 보내지 않던 점이었다. 초기 shell HTML은 이 값을 빈 문자열로 시작하기 때문에, webview는 이후 snapshot을 여러 번 받아도 계속 `''`를 current editable file로 들고 있게 됐다.
- 이 상태에서는 `webviewsrc/focustree.ts`의 `isContinuousFocusEditable()`가 항상 false가 될 수 있었고, optimistic create 경로도 placeholder source file을 잘못 잡을 수 있었다. 그래서 지속 중점 이동이 막히고, 편집 가능 상태가 preview에 즉시 반영되지 않는 회귀가 생겼다.
- `src/previewdef/focustree/renderpayloadpatch.ts`와 `src/previewdef/focustree/webviewupdate.ts`는 이제 partial/full snapshot update 모두에 `focusPositionActiveFile`을 실어 보낸다. `webviewsrc/focustree/messageapply.ts`와 `webviewsrc/focustree.ts`도 그 값을 runtime 상태와 `window` 양쪽에 동기화해서 이후 editability 판정이 실제 파일 기준으로 유지되도록 맞췄다.
- `test/unit/focustree-messageapply.test.ts`는 snapshot update가 active file을 적용하는지 고정했고, `test/unit/focustree-renderpayloadpatch.test.ts`는 partial update payload에도 active file이 포함되는지 확인한다. `test/unit/focustree-edithandler.test.ts`에는 continuous focus position ack 회귀도 추가했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-messageapply.test.js out\\test\\unit\\focustree-renderpayloadpatch.test.js out\\test\\unit\\focustree-edithandler.test.js out\\test\\unit\\focustree-previewsession.test.js` passed with 23 tests.

# FocusTree Pending Placeholder Card Parity Fix 2026-04-12

## Plan
- [x] pending placeholder와 일반 focus 카드가 어떤 렌더 경로에서 갈라지는지 다시 확인한다
- [x] 새 placeholder가 별도 SVG 대신 공용 focus 카드 템플릿과 스타일을 재사용하도록 수정한다
- [x] 빈 트리에서도 공용 focus 카드 스타일이 먼저 등록되도록 보강한다
- [x] 관련 compile/test를 다시 실행한다

## Review
- 원인은 `src/previewdef/focustree/localpreview.ts`의 pending placeholder가 여전히 별도 SVG 템플릿을 쓰고 있어서, 실제 focus 카드와 DOM 구조와 스타일 경로가 계속 분리되어 있던 점이었다. 그래서 새로 만든 focus는 authoritative host refresh가 오기 전까지 기존 focus와 다른 모양으로 보였다.
- `src/previewdef/focustree/focusrender.ts`는 이제 `ensureFocusTemplateStyles()`로 공용 카드 클래스 등록을 분리하고, 일반 focus 렌더와 pending placeholder가 같은 카드 구조를 재사용하도록 정리했다.
- `src/previewdef/focustree/localpreview.ts`의 `renderPendingPlaceholderFocusTemplate()`은 더 이상 별도 SVG를 만들지 않고, 완전한 placeholder `Focus` shape를 구성해 `renderFocusHtmlTemplate()`로 바로 렌더한다. 덕분에 새 focus도 생성 직후부터 기존 카드와 같은 구조, 같은 `{{iconClass}}` 교체 경로를 탄다.
- `src/previewdef/focustree/contentbuilder.ts`는 실제 focus가 아직 하나도 없어도 공용 focus 카드 스타일을 먼저 동적 CSS에 등록한다. 그래서 빈 트리에서 첫 focus를 만들 때도 pending 카드가 무스타일 상태로 뜨지 않는다.
- `test/unit/focustree-localpreview.test.ts`는 pending placeholder가 실제 카드 구조와 editable/source-file metadata를 유지하는지로 기대값을 바꿨고, `test/unit/focustree-contentbuilder.test.ts`는 공용 카드 CSS가 빈 tree payload에서도 항상 존재하는지 회귀 테스트를 추가했다.

## Verification
- `npm run compile-tests` passed.
- `npm run compile-ts` passed.
- `node .\\node_modules\\mocha\\bin\\mocha --exit out\\test\\unit\\focustree-localpreview.test.js out\\test\\unit\\focustree-contentbuilder.test.js out\\test\\unit\\focustree-focusrender.test.js` passed with 10 tests.
