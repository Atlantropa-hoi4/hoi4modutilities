...
import type { ModifierDefinitions } from "../../src/util/modifiers";
...
nodeModule._load = function(request: string, parent: NodeModule | undefined, isMain: boolean) {
	if (request === 'vscode') {
		return { env: { language: 'en' }, l10n: { bundle: {}, t: (message: string) => message }, workspace: { getConfiguration: () => ({ featureFlags: ['!localisationIndex'] }) } };
	}
...
const {
	...
	resolveDefinition,
	toneFor,
} = require("../../src/util/modifiers") as typeof import("../../src/util/modifiers");
...
describe("previewdef/idea/modifiers research bonuses", () => {
	...
	it("reads a negative research bonus as bad", async () => {
		const lines = await formatResearchBonuses([{ key: "armor", value: -0.1 }], false);
		assert.strictEqual(lines[0]?.tone, "bad");
	});

	it("makes the category readable when nothing localises it", async () => {
		const lines = await formatResearchBonuses([{ key: "CAT_fuel_oil", value: 0.05 }], false);
