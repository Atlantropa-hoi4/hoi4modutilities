...
export async function renderDropdownBox(dropdownBox: HOIPartial<DropdownBoxType>, parentInfo: ParentInfo, options: RenderDropdownBoxOptions): Promise<string> {
    ...
    return `<div start="${dropdownBox._token?.start}" end="${dropdownBox._token?.end}" class="gui-dropdown ${options.styleTable.style('positionAbsolute', () => `position: absolute;`)} ${options.styleTable.oneTimeStyle('dropdownbox', () => `left:${x}px;top:${y}px;width:${width}px;height:${height}px;overflow:visible;`)} ${options.enableNavigator ? 'navigator navigator-highlight' : ''}">${children}</div>`;
}
