...
export async function renderEditBox(editBox: HOIPartial<EditBoxType>, parentInfo: ParentInfo, options: RenderNodeCommonOptions): Promise<string> {
    ...
    return `<div start="${editBox._token?.start}" end="${editBox._token?.end}" class="${options.styleTable.style('positionAbsolute', () => `position:absolute;`)} ${options.styleTable.style('borderBox', () => `box-sizing:border-box;`)} ${options.styleTable.oneTimeStyle('editbox', () => `left:${x}px;top:${y}px;width:${width}px;height:${height}px;overflow:hidden;`)} ${options.enableNavigator ? 'navigator navigator-highlight' : ''}">${content}</div>`;
}
