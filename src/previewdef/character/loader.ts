...
export class CharactersLoader extends ContentLoader<CharactersLoaderResult> {
	...
	protected async postLoad(
		...
		return {
			...
			dependencies: uniq([this.file, ...traits.files, ...definitionFiles, ...gfxFiles,
                ...characters.characters.flatMap(c => c.portraits).filter(p => !isSpriteName(p.value)).map(p => p.value.replace(/\\+/g, "/"))]),
		};
	}
